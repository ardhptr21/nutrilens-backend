...
class NutritionService {
  public async scan(data: ScanModel["body"]) {
    ...
    if (!result.text)
      return ServiceResponse.failure(
        "Failed to generate nutrition data",
        null,
        StatusCodes.INTERNAL_SERVER_ERROR
      );
  ...
  public async getStatistics(userId: string, date: Date) {
    try {
      ...
      const nutrition =
        await nutritionRepository.findNutritionWithMealsByUserIdAndDate(
          userId,
          startOfDay,
          endOfDay
        );
      ...
      if (!nutrition) {
        ...
        if (startOfDay.getTime() === today.getTime()) {
          const userPreference = await preferenceRepository.findByUserId(
            userId
          );
  ...
  public async uploadMeal(data: MealUploadModel["body"], userId: string) {
    try {
      ...
      if (!userPreference) {
        return ServiceResponse.failure(
          "User preference not found. Please setup your nutrition preferences first",
          null,
      ...
      let nutrition = await nutritionRepository.findNutritionByUserIdAndDate(
        userId,
        today
      );
